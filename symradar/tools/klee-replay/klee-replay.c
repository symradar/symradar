...
static void usage(void) {
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "Use KLEE_REPLAY_TIMEOUT environment variable to set a "
                  "timeout (in seconds).\n");
  exit(1);
}
...
int main(int argc, char **argv) {
  ...
  int c, opt_index;
  while ((c = getopt_long(argc, argv, "f:r:", long_options, &opt_index)) !=
         -1) {
    switch (c) {
    case 'f': {
  ...
  if (rootdir && strstr(executable, rootdir) != executable) {
    fprintf(stderr,
            "Error: chroot: root dir should be a parent dir of executable.\n");
    exit(1);
  }
...
void klee_warning(char *name) { fprintf(stderr, "WARNING: %s\n", name); }

void klee_warning_once(char *name) { fprintf(stderr, "WARNING: %s\n", name); }
