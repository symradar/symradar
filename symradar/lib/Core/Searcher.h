...
class UniKLEESearcher : public Searcher {
  std::vector<ExecutionState *> states;
  ExecutionState *crashState;
  Executor &executor;
  bool mergeRequired;
  enum StateType targetType;

public:
  UniKLEESearcher(Executor &_executor);
  ~UniKLEESearcher();
  ExecutionState &selectState();
  void update(ExecutionState *current,
              const std::vector<ExecutionState *> &addedStates,
              const std::vector<ExecutionState *> &removedStates);
  bool empty() { return states.empty(); }
  void printName(llvm::raw_ostream &os) { os << "UniKLEESearcher\n"; }
};
