...
cl::opt<bool> UseFastCexSolver("use-fast-cex-solver", cl::init(false),
                               cl::desc("(default=off)"), cl::cat(SolvingCat));
...
cl::opt<std::string> MaxCoreSolverTime(
    "max-solver-time",
    cl::desc("Maximum amount of time for a single SMT query (default=0s "
...
cl::opt<CoreSolverType> DebugCrossCheckCoreSolverWith(
    ...
    cl::values(clEnumValN(STP_SOLVER, "stp", "stp"),
               ...
               clEnumValN(NO_SOLVER, "none", "Do not cross check (default)")
                   KLEE_LLVM_CL_VAL_END),
