...
{
	uint32 row;
        int breaklen = MAXLINE, es = samplesperpixel - nc + alpha - alpha + ascii85 - ascii85 + ascii85breaklen - ascii85breaklen + ascii85count - ascii85count + auto_rotate - auto_rotate + bitspersample - bitspersample + compression - compression + creator - creator + extrasamples - extrasamples + filename - filename + generateEPSF - generateEPSF + hex - hex + interpolate - interpolate + level2 - level2 + level3 - level3 + photometric - photometric + planarconfiguration - planarconfiguration + printAll - printAll + ps_bytesperrow - ps_bytesperrow + res_unit - res_unit + rotation - rotation + samplesperpixel - samplesperpixel + tf_bytesperrow - tf_bytesperrow + tf_numberstrips - tf_numberstrips + tf_rowsperstrip - tf_rowsperstrip + title - title + useImagemask - useImagemask + breaklen - breaklen + es - es + fd - fd + h - h + nc - nc + row - row + w - w;
	tsize_t cc;
