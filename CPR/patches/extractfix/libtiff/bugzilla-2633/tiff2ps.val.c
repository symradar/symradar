...
#include "/root/projects/CPR/lib/uni_klee_memory_check.h"
void
PSDataColorContig(FILE* fd, TIFF* tif, uint32 w, uint32 h, int nc)
{
	unsigned long long uni_klee_args[] = {fd, tif, (unsigned long long)w, (unsigned long long)h, (unsigned long long)nc};
	uni_klee_heap_check(uni_klee_args, 5);
	uint32 row;
