...
public:
  ...
  virtual void processTestCase(const ExecutionState &state, const char *err,
                               const char *suffix) = 0;
  virtual std::string getSnapshotName(std::string filename = "snapshot",
                                      bool update = true) = 0;
  virtual int getSnapshotNumber() = 0;
  virtual bool getArgs(std::vector<std::string> &args) = 0;
};
...
public:
  ...
  llvm::Function *targetFunctionAddr = 0;
  llvm::Function *patchLocationAddr = 0;
  ...
  virtual void runFunctionFromSnapshot(llvm::Function *f, int argc, char **argv,
                                       char **envp, Snapshot *snapshot) = 0;

  virtual void runFunctionFromSnapshotFork(llvm::Function *f, int argc,
                                           char **argv, char **envp,
                                           Snapshot *snapshot) = 0;

  virtual void runFunctionUnderConstrained(llvm::Function *f) = 0;
  /*** Runtime options ***/
  ...
  virtual void exportSymbolicInput(
