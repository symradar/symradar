...
typedef ImmutableMap<const MemoryObject *, ObjectHolder, MemoryObjectLT>
    MemoryMap;
...
public:
  ...
  AddressSpace() : cowKey(1), initialized(false) {}
  AddressSpace(const AddressSpace &b)
      : cowKey(++b.cowKey), objects(b.objects), initialized(b.initialized) {}
  ~AddressSpace() {}

  void init(const AddressSpace &b) {
    cowKey = ++b.cowKey;
    objects = b.objects;
    initialized = true;
  }

  void mergeWithShadow(const AddressSpace &shadow) {
    // copy
    cowKey = ++shadow.cowKey;
    objects = shadow.objects;
    initialized = false;
  }
  ...
  ObjectState *copyObjectState(const MemoryObject *mo, const ObjectState *os);
