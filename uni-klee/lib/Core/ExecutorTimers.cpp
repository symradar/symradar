...
cl::opt<std::string> MaxTime("max-time",
                             cl::desc("Halt execution after the specified "
                                      "number of seconds (default=0s (off))"));
...
static void onAlarm(int) { ++timerTicks; }
...
void Executor::processTimers(ExecutionState *current, time::Span maxInstTime) {
    ...
    if (maxInstTime && current &&
      ...
      if (timerTicks * kMilliSecondsPerTick > maxInstTime) {
        klee_warning("max-instruction-time exceeded: %.2fs",
                     (timerTicks * kMilliSecondsPerTick).toSeconds());
