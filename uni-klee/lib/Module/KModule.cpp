...
cl::opt<std::string> SlicingInfoFile("slicing-info",
                                     cl::desc("path to sliced line file"));
...
cl::opt<SwitchImplType> SwitchType(
    "switch-type", cl::desc("Select the implementation of switch"),
...
void KModule::manifest(InterpreterHandler *ih, bool forceSourceOutput) {
  ...
  if (!SlicingInfoFile.empty()) {
    std::ifstream csvFile(SlicingInfoFile.c_str());
    std::string line;
    while (getline(csvFile, line, ',')) {
      slicedLines.push_back(atoi(line.c_str()));
    }
  }
  ...
  for (auto &Function : *module) {
    ...
    for (unsigned i = 0; i < kf->numInstructions; ++i) {
      ...
      if (!slicedLines.empty() &&
          std::find(slicedLines.begin(), slicedLines.end(),
                    ki->info->assemblyLine) != slicedLines.end()) {
        ki->isSliced = true;
      }
    }
