...
getIndependentConstraints(const Query &query, std::vector<ref<Expr>> &result) {
  ...
  KLEE_DEBUG(std::set<ref<Expr>> reqset(result.begin(), result.end());
             errs() << "--\n"; errs() << "Q: " << query.expr << "\n";
             errs() << "\telts: " << IndependentElementSet(query.expr) << "\n";
             int i = 0; for (ConstraintManager::const_iterator it =
               ...
               errs() << "\telts: " << IndependentElementSet(*it) << "\n";
                      ...
                      << eltsClosure << "\n";);
  // std::string ds;
  // llvm::raw_string_ostream dss(ds);
  // std::set<ref<Expr>> reqset(result.begin(), result.end());
  // dss << "Q: " << query.expr << "\n";
  // dss << "\telts: " << IndependentElementSet(query.expr) << "\n";
  // int i = 0;
  // for (ConstraintManager::const_iterator it = query.constraints.begin(),
  //                                        ie = query.constraints.end();
  //      it != ie; ++it) {
  //   dss << "C" << i++ << ": " << *it;
  //   dss << " " << (reqset.count(*it) ? "(required)" : "(independent)") <<
  //   "\n"; dss << "\telts: " << IndependentElementSet(*it) << "\n";
  // }
  // dss << "elts closure: " << eltsClosure << "\n";
  // SPDLOG_DEBUG("{}", dss.str());
  return eltsClosure;
}
