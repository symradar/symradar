...
SUBJECTS={
    ...
    'binutils/cve_2018_10372': 125,
    'coreutils/bugzilla_19784': 787,
    'coreutils/bugzilla_26545': 787,
    'coreutils/gnubug_25003': 125,
    'coreutils/gnubug_25023': 125,
    'jasper/cve_2016_8691': 369,
    'jasper/cve_2016_9387': 190,
    'libjpeg/cve_2012_2806': 787,
    'libjpeg/cve_2017_15232': 476,
    'libjpeg/cve_2018_14498': 125,
    'libjpeg/cve_2018_19664': 125,
    'libtiff/bugzilla_2611': 369,
    'libtiff/bugzilla_2633': 125,
    'libtiff/cve_2014_8128': 787,
    'libtiff/cve_2016_3186': 119,
    'libtiff/cve_2016_3623': 369,
    'libtiff/cve_2016_5314': 787,
    'libtiff/cve_2016_5321': 119,
    'libtiff/cve_2016_9273': 125,
    'libtiff/cve_2016_10094': 189,
    'libtiff/cve_2017_7595': 369,
    'libtiff/cve_2017_7601': 20,
    'libxml2/cve_2012_5134': 119,
    'libxml2/cve_2016_1834': 119,
    'libxml2/cve_2016_1838': 125,
    'libxml2/cve_2016_1839': 125,
    'libxml2/cve_2017_5969': 476,
}
benchmark_dir='/root/project/VulMaster/benchmarks/extractfix'
...
def parse_code_tokens(candidate, lang):
    ...
    candidate = remove_comments_and_docstrings(candidate, tscsharp)
...
def get_token_pair_diff(pre_version_file_str,
    ...
    try:
        ...
        if diff:
            ...
            print(f'No diff found during parsing CWE examples for {pre_version_file_str}')
            print('You can just ignore it!')
            sys.exit(2)
    except Exception as e:
        print("Get token pair fail: " + str(e))

def add_special_space_token (src):
    src_tokens = parse_code_tokens(src, tscsharp)
    new_src_tokens = []
...
def generate_SBT(candidate, lang):
    JAVA_LANGUAGE = Language(lang.language())
    parser = Parser()
...
# Parse the AST and diff
print('Parsing the ASTs and AST diffs...')
start_end_positions = []
ast_data = []
...
for sub in SUBJECTS:
    ...
    with open(os.path.join(benchmark_dir,sub, buggy_file), 'r') as f:
        buggy_source = f.read()
    start_end_pos= find_buggy_location(buggy_source)
    _ast_data=buggy_source
    start_end_positions.append(start_end_pos)
    ast_data.append(_ast_data)

for i in tqdm(range(len(ast_data))):
    code = ast_data[i]
    # sbt_exp, type_ = generate_SBT(code, tscsharp)
    # Question 2: Why you use C# only? Where is C?
    first_order  = generate_deepth_first_trajectory(code, tscsharp)
    subtrees = generate_subtree(code , start_end_positions[i],  tscsharp)
    all_first_trees.append(first_order)
    all_trees.append(subtrees)

# Parse cwe tree
print('Parsing the CWE tree...')
...
for i,sub in enumerate(SUBJECTS):
    ...
    with open(os.path.join(benchmark_dir,sub, buggy_file), 'r') as f:
        buggy_source = f.read()
    with open(os.path.join(benchmark_dir,sub, correct_file), 'r') as f:
...
with open('extractfix.json', 'w') as f:
    json.dump(test_fid_input_lists, f, indent=4)
print('Dataset generated successfully in extractfix.json!')