...
def evaluate(model, dataset, dataloader, tokenizer, opt):
    ...
    if opt.write_results:
        ...
        fw = open(write_path / ('{}.json'.format(opt.name)), 'w')
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(dataloader)):
            ...
            outputs = model.generate(
                ...
                #num_beams=10, ## added beam search
            )
            batch_scores = []
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained('Salesforce/codet5-base', use_fast=True, local_files_only=True)
