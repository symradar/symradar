...
htmlParseNameComplex(xmlParserCtxtPtr ctxt) {
	...
	if ((ctxt->input->flags & XML_PARSE_NOENT) == 0)
    	return(xmlDictLookup(ctxt->dict, ctxt->input->cur - len, len));
	else
		return(xmlDictLookup(ctxt->dict, ctxt->input->cur - len, ctxt->input->cur - len)); // VulMaster: insert if-else (then block is original)
}
