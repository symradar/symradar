...
public class FunctionMatcher {
	...
	private List<FunctionMap> commonFunctions = null;
	 ...
	 */
	public void match(){
		this.commonFunctions = new ArrayList<FunctionMap>();
		switch (this.method) {
		case AST_MATCHING:
			if(this.mapper == null)
	 ...
	 */
	public List<FunctionMap> getCommonFunctions(){
	...
	public boolean isOldFuncMatched(FunctionDef currd) {
		for(FunctionMap cm:this.commonFunctions) {
			if(cm.getOldFunction() == currd) {
				return true;
			}
		}
		return false;
	...
	public boolean isNewFuncMatched(FunctionDef currd) {
		for(FunctionMap cm:this.commonFunctions) {
			if(cm.getNewFunction() == currd) {
				return true;
			}
		}
		return false;
		...
		for(FunctionDef oldFunc: oldFunctionList){
			for(FunctionDef newFunc : newFunctionList){
				if(oldFunc.name.getEscapedCodeStr().equals(newFunc.name.getEscapedCodeStr()))
					this.commonFunctions.add(new FunctionMap(oldFunc, newFunc));
			}
		}
	}
