 ...
 *
 */
public class FunctionDiffCheckMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(FunctionDiffCheckMain.preprocessFile(srcFile, srcPreprocessFile)) {
				logger.info("Preprocessed the src file:{} to {}", srcFile.getAbsolutePath(), srcPreprocessFile.getAbsolutePath());
			}
			if(FunctionDiffCheckMain.preprocessFile(dstFile, dstPreprocessFile)) {
			...
			// clean up
			if(FunctionDiffCheckMain.removePreProcess) {
	...
	private static boolean isPatchSafe(FunctionMap targetMap, PrintWriter jsonWriter) {
		...
		if(FunctionDiffCheckMain.checkUpdatedStmts(targetMap, currChecker, jsonWriter)) {
		...
		if(FunctionDiffCheckMain.onlyDeletePatch(targetMap)) {
