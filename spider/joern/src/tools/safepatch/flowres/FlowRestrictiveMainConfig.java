 ...
 *
 */
public class FlowRestrictiveMainConfig {
	...
	public static boolean ignoreErrorBB = false;
	public static boolean ignoreDeletedStatements = true;
	public static boolean ignoreInsertHeuristics = false;
	...
	public static void main(String[] args) {
		...
		if(args.length > 3) {
			String tarOp = args[3];
			boolean detec = false;
			if(tarOp.contains("insert")) {
				detec = true;
				FlowRestrictiveMainConfig.ignoreInsertHeuristics = false;
				logger.info("Enabling insert heuristics");
			}
			...
			if(tarOp.contains("delete")) {
				detec = true;
				logger.info("Enabling delete heuristics");
				FlowRestrictiveMainConfig.ignoreDeletedStatements = false;
			}
			if(!detec) {
				logger.error("Invalid option {} provided", tarOp);
				return;
			}
		...
		try {
			...
			if(FlowRestrictiveMainConfig.preprocessFile(srcFile, srcPreprocessFile)) {
				logger.info("Preprocessed the src file:{} to {}", srcFile.getAbsolutePath(), srcPreprocessFile.getAbsolutePath());
			}
			if(FlowRestrictiveMainConfig.preprocessFile(dstFile, dstPreprocessFile)) {
			...
			if(diffAnalyzableReason != null) {
				...
				if(diffFunctions.size() == 0) {
					...
					for(FunctionMap fmap:diffFunctions) {
						...
						printWriter.write("{");
						if(FlowRestrictiveMainConfig.isPatchSafe(fmap, printWriter)) {
			...
			// clean up
			if(FlowRestrictiveMainConfig.removePreProcess) {
	...
	private static boolean isPatchSafe(FunctionMap targetMap, PrintWriter jsonWriter) {
		...
		if(FlowRestrictiveMainConfig.checkUpdatedStmts(targetMap, currChecker, jsonWriter)) {
		...
		if(FlowRestrictiveMainConfig.onlyDeletePatch(targetMap) || (FlowRestrictiveMainConfig.ignoreDeletedStatements && FlowRestrictiveMainConfig.hasDeletedStatements(targetMap))) {
	...
	private static boolean hasDeletedStatements(FunctionMap targetMap) {
		for(StatementMap currMap:targetMap.getStatementMap()) {
			if(currMap.getMapType() == MAP_TYPE.DELETE) {
				return true;
			}
		}
		return false;
