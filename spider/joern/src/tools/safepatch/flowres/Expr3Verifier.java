...
public class Expr3Verifier {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(Expr3Verifier.preprocessFile(srcFile, srcPreprocessFile)) {
				logger.info("Preprocessed the src file:{} to {}", srcFile.getAbsolutePath(), srcPreprocessFile.getAbsolutePath());
			}
			if(Expr3Verifier.preprocessFile(dstFile, dstPreprocessFile)) {
			...
			if(diffAnalyzableReason != null) {
				...
				if(diffFunctions.size() == 0) {
					...
					for(FunctionMap fmap:diffFunctions) {
						...
						printWriter.write("{");
						if(Expr3Verifier.isPatchSafe(fmap, printWriter)) {
			...
			// clean up
			if(Expr3Verifier.removePreProcess) {
	...
	private static boolean isPatchSafe(FunctionMap targetMap, PrintWriter jsonWriter) {
		...
		if(Expr3Verifier.checkUpdatedStmts(targetMap, currChecker, jsonWriter)) {
		...
		if(!insertChecker.areAnyStatementsInserted(jsonWriter)) {
			logger.debug("No inserted statements.");
		} else {
			logger.debug("One or more inserted statements.");
			retVal = false;
		}
		
		// check moved statements.
		MovedStatementChecker movedChecker = new MovedStatementChecker(targetMap);
		if(!movedChecker.areAnyStatementsMoved(jsonWriter)) {
			logger.debug("No moved statements.");
		} else {
			logger.debug("One or more moved statements.");
			retVal = false;
		}
		
		if(Expr3Verifier.hasDeletedStatements(targetMap)) {
			logger.debug("Has deleted statements, we cannot handle this");
			retVal = false;
			jsonWriter.write("\"hasDelete\": \"true\",");
		} else {
			jsonWriter.write("\"hasDelete\": \"false\" ,");
		}
		...
		jsonWriter.write("\"likelysecurity\":" + Boolean.toString(retVal));
		return retVal;
	}
		...
		for(StatementMap currMap:targetMap.getStatementMap()) {
			if(currMap.getMapType() == MAP_TYPE.UPDATE && currMap.getOriginal() != null && currMap.getNew() != null) {
				...
				if(!targetChecker.isOldStmtHandled(oldSt) && !targetChecker.isNewStmtHandled(newSt)) {
					...
					retVal = false;
					break;
					/*if(!oldBB.isErrorHandling() && !newBB.isErrorHandling()) {
						retVal = false;
					} else {
