...
public class DominatorTree<V>
{
	...
	public Set<V> getAllDominators(V vertex, Set<V> visited) {
		...
		visited.add(vertex);
		V dom = getDominator(vertex);
		ret.add(dom);
		ret.addAll(getAllDominators(dom, visited));
	...
	private static class DominatorTreeCreator<V, E extends Edge<V>>
	{
		...
		private void enumerateVertices()
		{
			...
			{
				System.err.println("warning: incomplete control flow graph");
			}
		}
